  ...
  const renderizarDetalhes = async (orcamentos: number | number[]): Promise<void> => {
    ...
    if (elAccounts) {
      ...
      agruparPorConta(detalhe).forEach(({ conta, total }) => {
        ...
        // Adiciona evento de clique para navegar para lancamentos com filtro de conta
        card.addEventListener('click', () => {
          const encodedConta = encodeURIComponent(conta);
          window.location.href = `lancamentos.html?conta=${encodedConta}`;
        });
    ...
    if (elCategoriesCards) {
      ...
      topCategorias.forEach((item, idx) => {
        ...
        // Adiciona evento de clique para navegar para lancamentos com filtro de categoria
        card.addEventListener('click', () => {
          const encodedCategoria = encodeURIComponent(item.categoria);
          window.location.href = `lancamentos.html?categoria=${encodedCategoria}`;
        });
