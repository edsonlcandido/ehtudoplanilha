var Y=Object.defineProperty;var W=(s,e,t)=>e in s?Y(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var $=(s,e,t)=>W(s,typeof e!="symbol"?e+"":e,t);import{C as J,c as A,v as K,p as P,A as Z}from"./auth.js";import{r as Q}from"./user-menu.js";import{t as N,e as V,g as ee,a as te,i as ae,b as ne,c as se,d as oe,l as re,o as ie,f as ce,h as de}from"./fab-menu.js";import{SheetsService as le}from"./sheets.js";const ue=new J(A.pocketbaseUrl);A.isDevelopment&&(console.log("[PocketBase] Inicializado em modo desenvolvimento"),console.log("[PocketBase] URL:",A.pocketbaseUrl));typeof window<"u"&&(window.pb=ue);function k(){if(document.getElementById("logoutModal"))return;document.body.insertAdjacentHTML("beforeend",`
  ...
  `)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",k):k();function S(s){return Number(s).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}let R=[],q={};function me(s,e){const t=document.getElementById("summaryCards");if(!t)return;R=s||[],q=e||{};const o=R.slice().sort((a,r)=>r.orcamento-a.orcamento);t.className="financial-cards",t.innerHTML="",o.forEach(a=>{q[a.label]===!0?t.appendChild(ge(a)):t.appendChild(pe(a))}),t.querySelectorAll(".financial-card--inactive").forEach(a=>{const r=a;r.style.cursor="pointer",r.addEventListener("click",D)}),t.querySelectorAll(".financial-card__close").forEach(a=>{a.addEventListener("click",G)})}function ge(s){const e=document.createElement("div"),t=s.sum>=0?"financial-card--incomes":"financial-card--expenses";return e.className=`financial-card ${t}`,e.dataset.budget=s.label,e.dataset.sum=String(s.sum),e.dataset.incomes=String(s.incomes),e.dataset.expenses=String(s.expenses),e.dataset.orcamento=String(s.orcamento),e.innerHTML=`
    ...
    <div class="financial-card__value">${S(s.sum)}</div>
    ...
    <div class="financial-card__details financial-card__details--hidden">
      <div class="financial-card__detail">Receitas: ${S(s.incomes)}</div>
      <div class="financial-card__detail">Despesas: ${S(s.expenses)}</div>
    </div>
  ...
  `)}function he(s=[],e="orcamento",t=-5,o=35){const{startSerial:n,endSerial:c}=ee(t,o);return s.filter(a=>{const r=a&&a[e];return typeof r=="number"&&!Number.isNaN(r)&&r>=n&&r<=c})}function z(s=[],e="orcamento"){const t=new Map;return s.forEach(n=>{let c=n&&n[e],a=Number(c);if(!Number.isFinite(a)&&typeof c=="string"&&c.includes("/")){const E=c.split("/");if(E.length===3){const[L,w,C]=E.map(M=>Number(M));[L,w,C].every(Number.isFinite)&&(a=N(new Date(C,w-1,L)))}else if(E.length===2){const[L,w]=E.map(C=>Number(C));[L,w].every(Number.isFinite)&&(a=N(new Date(w,L-1,1)))}}if(!Number.isFinite(a))return;const r=Number(n.valor)||0,l=t.get(a)||{orcamento:a,sum:0,count:0,incomes:0,expenses:0};l.sum+=r,l.count+=1,r>=0?l.incomes+=r:l.expenses+=r,t.set(a,l)}),Array.from(t.values()).map(n=>({orcamento:n.orcamento,label:V(n.orcamento),count:n.count,sum:Number(n.sum.toFixed(2)),incomes:Number(n.incomes.toFixed(2)),expenses:Number(n.expenses.toFixed(2))})).sort((n,c)=>c.orcamento-n.orcamento)}function H(s=[],e="orcamento"){const t=n=>{let c=Number(n);if(Number.isFinite(c))return c;if(typeof n=="string"&&n.includes("/")){const a=n.split("/");if(a.length===3){const[r,l,E]=a.map(L=>Number(L));if([r,l,E].every(Number.isFinite))return N(new Date(E,l-1,r))}else if(a.length===2){const[r,l]=a.map(E=>Number(E));if([r,l].every(Number.isFinite))return N(new Date(l,r-1,1))}}return null},o=new Map;return s.forEach(n=>{const c=n&&n[e],a=t(c);a===null||!Number.isFinite(a)||o.has(a)||o.set(a,{orcamento:a,label:V(a)})}),Array.from(o.values()).sort((n,c)=>n.orcamento-c.orcamento)}function X(s=[]){const e=new Map;return s.forEach(o=>{const n=String(o.conta||"");if(!n||n.trim()==="")return;const c=Number(o.valor)||0,a=e.get(n)||{total:0,count:0};a.total+=c,a.count+=1,e.set(n,a)}),Array.from(e.entries()).map(([o,n])=>({conta:o,total:Number(n.total.toFixed(2)),count:n.count})).sort((o,n)=>o.conta.localeCompare(n.conta))}function B(s){return s.toLocaleString("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:2,maximumFractionDigits:2})}class ve{constructor(e){$(this,"container");$(this,"entries",[]);$(this,"chartData",[]);const t=document.getElementById(e);if(!t)throw new Error(`Container ${e} não encontrado`);this.container=t}render(e){if(this.entries=e,this.chartData=this.calculateExpenseData(),this.chartData.length===0){this.renderEmptyState();return}this.renderChart()}calculateExpenseData(){const e=["TRANSFERÊNCIA","TRANSFERENCIA","SALDO","RENDA","RECEITA"],t=this.entries.filter(a=>{const r=a.tipo.toUpperCase().trim();return!e.includes(r)&&a.valor<0});if(t.length===0)return[];const o=new Map;for(const a of t){const r=a.tipo||"Sem Tipo",l=Math.abs(a.valor);o.set(r,(o.get(r)||0)+l)}const n=Array.from(o.values()).reduce((a,r)=>a+r,0);return Array.from(o.entries()).map(([a,r])=>({categoria:a,valor:r,percentual:r/n*100})).sort((a,r)=>r.valor-a.valor)}renderEmptyState(){this.container.innerHTML=`
    ...
    `}renderDonutChart(e){const r=e.reduce((C,M)=>C+M.valor,0);if(r===0)return"<p>Sem dados de despesas</p>";let l=-90;return`
      ...
      >
        ${e.map((C,M)=>{const d=C.valor/r*100/100*360,u=l,g=l+d;l=g;const h=u*Math.PI/180,_=g*Math.PI/180,m=100+85*Math.cos(h),i=100+85*Math.sin(h),f=100+85*Math.cos(_),p=100+85*Math.sin(_),y=d>180?1:0,v=["M 100 100",`L ${m} ${i}`,`A 85 85 0 ${y} 1 ${f} ${p}`,"Z"].join(" "),b=this.getColor(M);return`
        <path 
          d="${v}" 
          fill="${b}" 
          stroke="white" 
...
`;async function be(s,e){const t=document.querySelector(".details");if(!t)return;let o=e.map(d=>d.orcamento),n=s||[];const c="excludedAccounts",a=()=>{try{const d=localStorage.getItem(c);if(d)return new Set(JSON.parse(d))}catch(d){console.error("Erro ao carregar contas excluídas:",d)}return new Set},r=d=>{try{localStorage.setItem(c,JSON.stringify(Array.from(d)))}catch(u){console.error("Erro ao salvar contas excluídas:",u)}},l=a(),E=async(d,u=0)=>{try{if(console.log("[Details] Preparando gráfico de despesas por tipo..."),!document.getElementById("categoryBudgetChart"))if(u<3){console.warn(`[Details] Container categoryBudgetChart não encontrado (tentativa ${u+1}/3), aguardando...`),setTimeout(()=>E(d,u+1),200);return}else{console.warn("[Details] Container categoryBudgetChart não encontrado após múltiplas tentativas. Gráfico não será renderizado.");return}console.log("[Details] Container encontrado, carregando categorias completas...");const _=await le.getSheetCategoriesComplete();if(!_||_.length===0){console.log("[Details] Nenhuma categoria completa encontrada");return}console.log("[Details] Fazendo JOIN entre entries e categoriesComplete...");const m=new Map;for(const v of _)m.set(v.categoria.toLowerCase(),v.tipo);console.log("[Details] Categorias mapeadas:",Array.from(m.entries()));const i=n.filter(v=>d.includes(v.orcamento));console.log(`[Details] Filtrando ${n.length} entries por ${d.length} orçamentos -> ${i.length} entries`);const f=[...new Set(i.map(v=>v.categoria).filter(v=>v))];console.log("[Details] 📋 Categorias presentes nos entries filtrados:",f);const p=i.map(v=>{const b=(v.categoria||"").toLowerCase(),T=m.get(b)||"Sem Tipo";return{categoria:v.categoria||"",valor:v.valor||0,tipo:T}}),y=p.filter(v=>v.tipo==="Sem Tipo");if(y.length>0){console.warn(`[Details] ⚠️ Encontrados ${y.length} entries SEM TIPO:`),console.table(y.map(b=>({categoria:b.categoria,valor:b.valor,tipo:b.tipo})));const v=[...new Set(y.map(b=>b.categoria))];console.warn("[Details] 📋 Categorias SEM TIPO encontradas:",v),v.forEach(b=>{const T=m.has(b);console.warn(`[Details] Categoria "${b}" existe no mapa? ${T}`)})}console.log("[Details] Renderizando gráfico de despesas por tipo..."),ye("categoryBudgetChart",p),console.log("[Details] ✅ Gráfico de despesas por tipo renderizado com sucesso")}catch(h){console.error("[Details] Erro ao renderizar gráfico de despesas:",h)}},L=()=>{const d=document.querySelector("#detail-saldo"),u=document.querySelector("#detail-accounts-cards"),g=document.querySelector(".dashboard__balance-section .details__aggregates"),h=window.accountSummary||[];console.log("🎨 Renderizando todas as contas:",h),g&&g.classList.remove("loading");const _=h.reduce((m,i)=>l.has(i.conta)?m:m+i.total,0);if(d&&(d.textContent=S(_)),u){u.innerHTML="",h.forEach(({conta:i,total:f})=>{const p=document.createElement("div"),y=l.has(i);p.className=`details__card details__card--clickable${y?" details__card--excluded":""}`,p.dataset.conta=i;const v=document.createElement("div");v.className="details__card-content",v.innerHTML=`
        ...
        `,p.appendChild(v),p.addEventListener("click",()=>{const b=v.querySelector(".details__card-icon");l.has(i)?(l.delete(i),p.classList.remove("details__card--excluded"),b&&(b.textContent="")):(l.add(i),p.classList.add("details__card--excluded"),b&&(b.textContent="💳")),r(l);const T=h.reduce((I,F)=>l.has(F.conta)?I:I+F.total,0);d&&(d.textContent=S(T))}),u.appendChild(p)});const m=document.querySelector("#toggle-accounts-btn");if(m&&u){const i=()=>{const y=u.classList.contains("details__cards--hidden");m.textContent=y?"👁️ Mostrar contas":"🙈 Ocultar contas"},f=m._toggleListener;f&&m.removeEventListener("click",f);const p=()=>{u.classList.toggle("details__cards--hidden"),i()};m._toggleListener=p,m.addEventListener("click",p),i()}}},w=d=>{const u={};return d.forEach(g=>{const h=g.categoria||"Sem categoria";u[h]=(u[h]||0)+(g.valor||0)}),Object.entries(u).map(([g,h])=>({categoria:g,total:h}))},C=d=>{const u=document.querySelector("#detail-budget-accounts-cards"),g=document.querySelector("#detail-budget-total");if(!u)return;const h=n.filter(i=>d.includes(i.orcamento)),_=h.reduce((i,f)=>i+f.valor,0);g&&(g.textContent=S(_));const m=X(h);if(u.innerHTML="",m.length===0){u.innerHTML='<p class="category-entries-empty">Nenhuma conta encontrada nos orçamentos selecionados.</p>';return}m.forEach(({conta:i,total:f})=>{const p=document.createElement("div");p.className="details__card",p.innerHTML=`
        <div class="details__card-info">
          <span class="details__card-title">${i}</span>
          <span class="details__card-value">${S(f)}</span>
        </div>
      `,u.appendChild(p)})},M=(d,u)=>{const g=t.querySelector("#detail-entries"),h=t.querySelector("#detail-entries-title"),_=t.querySelector("#entries-list");if(!g||!_||!h)return;h.innerHTML=`<span id="lancamentos">Lançamentos da Categoria: ${d}</span>`;const m=n.filter(i=>u.includes(i.orcamento)&&(i.categoria||"Sem categoria")===d&&i.valor<0);if(m.sort((i,f)=>{if(!i.data&&!f.data)return 0;if(!i.data)return 1;if(!f.data)return-1;const p=new Date(i.data).getTime(),y=new Date(f.data).getTime();return isNaN(p)&&isNaN(y)?0:isNaN(p)?1:isNaN(y)?-1:y-p}),g.classList.remove("details__category-entries--hidden"),_.innerHTML="",m.length===0){_.innerHTML='<p class="category-entries-empty">Nenhum lançamento encontrado nesta categoria.</p>';return}m.forEach(i=>{const f=document.createElement("div");f.className="category-entry-card";let p="--";if(i.data&&typeof i.data=="number"&&i.data>0){const y=te(i.data,!0);y&&(p=y.toLocaleDateString("pt-BR")+" "+y.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}))}f.innerHTML=`
        <div class="category-entry-card__date">${p}</div>
        <div class="category-entry-card__description">${i.descricao||"Sem descrição"}</div>
        <div class="category-entry-card__value">${S(i.valor||0)}</div>
      `,_.appendChild(f)}),setTimeout(()=>{const i=document.getElementById("lancamentos");i&&i.scrollIntoView({behavior:"smooth",block:"start"})},100)},x=async d=>{const u=Array.isArray(d)?d:[d];t.innerHTML=_e,t.style.display="";const g=t.querySelector("#detail-categories-cards"),h=n.filter(_=>u.includes(_.orcamento));g&&(g.innerHTML="",(h.length>0?w(h).filter(m=>m.total<0).sort((m,i)=>m.total-i.total).slice(0,10):[]).forEach((m,i)=>{const f=document.createElement("div");f.className="category-card",f.dataset.categoria=m.categoria,f.innerHTML=`
