import{v as k,p as I,A as F}from"./auth.js";import"./sheets.js";import{e as N,t as S,a as q,g as A,i as D,b as H,c as R,d as z,l as V,o as O,f as P,h as j}from"./fab-menu.js";import{r as G}from"./user-menu.js";function E(a){return Number(a).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}let w=[],M={};function U(a,e){const r=document.getElementById("summaryCards");if(!r)return;w=a||[],M=e||{};const s=w.slice().sort((t,o)=>o.orcamento-t.orcamento);r.className="financial-cards",r.innerHTML="",s.forEach(t=>{M[t.label]===!0?r.appendChild(W(t)):r.appendChild(J(t))}),r.querySelectorAll(".financial-card--inactive").forEach(t=>{const o=t;o.style.cursor="pointer",o.addEventListener("click",L)}),r.querySelectorAll(".financial-card__close").forEach(t=>{t.addEventListener("click",$)})}function W(a){const e=document.createElement("div"),r=a.sum>=0?"financial-card--incomes":"financial-card--expenses";return e.className=`financial-card ${r}`,e.dataset.budget=a.label,e.dataset.sum=String(a.sum),e.dataset.incomes=String(a.incomes),e.dataset.expenses=String(a.expenses),e.dataset.orcamento=String(a.orcamento),e.innerHTML=`
...
`;function Y(a,e){const r=document.querySelector(".details");if(!r)return;let s=e.map(d=>d.orcamento),n=a||[];const c=d=>{const m={};return d.forEach(i=>{!i.conta||i.conta.trim()===""||(m[i.conta]=(m[i.conta]||0)+(i.valor||0))}),Object.entries(m).map(([i,y])=>({conta:i,total:y}))},t=d=>{const m={};return d.forEach(i=>{const y=i.categoria||"Sem categoria";m[y]=(m[y]||0)+(i.valor||0)}),Object.entries(m).map(([i,y])=>({categoria:i,total:y}))},o=(d,m)=>{const i=r.querySelector("#detail-entries"),y=r.querySelector("#detail-entries-title"),_=r.querySelector("#entries-list");if(!i||!_||!y)return;y.textContent=`Lançamentos da Categoria: ${d}`;const h=n.filter(l=>m.includes(l.orcamento)&&(l.categoria||"Sem categoria")===d&&l.valor<0);if(h.sort((l,f)=>{if(!l.data&&!f.data)return 0;if(!l.data)return 1;if(!f.data)return-1;const u=new Date(l.data).getTime(),g=new Date(f.data).getTime();return isNaN(u)&&isNaN(g)?0:isNaN(u)?1:isNaN(g)?-1:g-u}),i.classList.remove("details__category-entries--hidden"),_.innerHTML="",h.length===0){_.innerHTML='<p class="category-entries-empty">Nenhum lançamento encontrado nesta categoria.</p>';return}h.forEach(l=>{const f=document.createElement("div");f.className="category-entry-card";let u="--";if(l.data&&typeof l.data=="number"&&l.data>0){const g=N(l.data,!0);g&&(u=g.toLocaleDateString("pt-BR"))}f.innerHTML=`
      ...
      `,_.appendChild(f)})},v=(d,m)=>{const i=r.querySelector("#detail-entries"),y=r.querySelector("#detail-entries-title"),_=r.querySelector("#entries-list");if(!i||!_||!y)return;y.textContent=`Lançamentos da Conta: ${d}`;const h=n.filter(l=>m.includes(l.orcamento)&&l.conta===d);if(h.sort((l,f)=>{if(!l.data&&!f.data)return 0;if(!l.data)return 1;if(!f.data)return-1;const u=new Date(l.data).getTime(),g=new Date(f.data).getTime();return isNaN(u)&&isNaN(g)?0:isNaN(u)?1:isNaN(g)?-1:g-u}),i.classList.remove("details__category-entries--hidden"),_.innerHTML="",h.length===0){_.innerHTML='<p class="category-entries-empty">Nenhum lançamento encontrado nesta conta.</p>';return}h.forEach(l=>{const f=document.createElement("div");f.className="category-entry-card";let u="--";if(l.data&&typeof l.data=="number"&&l.data>0){const g=N(l.data,!0);g&&(u=g.toLocaleDateString("pt-BR"))}f.innerHTML=`
        <div class="category-entry-card__date">${u}</div>
        <div class="category-entry-card__description">${l.descricao||"Sem descrição"}</div>
        <div class="category-entry-card__value">${E(l.valor||0)}</div>
      `,_.appendChild(f)})},p=d=>{const m=Array.isArray(d)?d:[d];r.innerHTML=X,r.style.display="";const i=r.querySelector("#detail-saldo"),y=r.querySelector("#detail-accounts-cards"),_=r.querySelector("#detail-categories-cards"),h=n.filter(f=>m.includes(f.orcamento));if(!h.length){i&&(i.textContent=E(0));return}const l=h.reduce((f,u)=>f+(u.valor||0),0);i&&(i.textContent=E(l)),y&&(y.innerHTML="",c(h).forEach(({conta:f,total:u})=>{const g=document.createElement("div");g.className="details__card details__card--clickable",g.dataset.conta=f,g.innerHTML=`
          <div class="details__card-title">${f}</div>
          <div class="details__card-value">${E(u)}</div>
        `,g.addEventListener("click",()=>{y.querySelectorAll(".details__card").forEach(C=>{C.classList.remove("details__card--selected")});const b=r.querySelector("#detail-categories-cards");b&&b.querySelectorAll(".category-card").forEach(C=>{C.classList.remove("category-card--selected")}),g.classList.add("details__card--selected"),v(f,m)}),y.appendChild(g)})),_&&(_.innerHTML="",t(h).filter(u=>u.total<0).sort((u,g)=>u.total-g.total).slice(0,10).forEach((u,g)=>{const b=document.createElement("div");b.className="category-card",b.dataset.categoria=u.categoria,b.innerHTML=`
