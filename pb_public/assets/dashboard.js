var V=Object.defineProperty;var P=(t,e,n)=>e in t?V(t,e,{enumerable:!0,configurable:!0,writable:!0,value:n}):t[e]=n;var w=(t,e,n)=>P(t,typeof e!="symbol"?e+"":e,n);import{v as O,p as A,A as X}from"./auth.js";import{SheetsService as j}from"./sheets.js";import{e as M,t as T,a as R,g as G,i as Y,b as U,c as W,d as J,l as Z,o as K,f as Q,h as ee}from"./fab-menu.js";import{r as te}from"./user-menu.js";function S(t){return Number(t).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}let x=[],B={};function ae(t,e){const n=document.getElementById("summaryCards");if(!n)return;x=t||[],B=e||{};const i=x.slice().sort((a,s)=>s.orcamento-a.orcamento);n.className="financial-cards",n.innerHTML="",i.forEach(a=>{B[a.label]===!0?n.appendChild(ne(a)):n.appendChild(re(a))}),n.querySelectorAll(".financial-card--inactive").forEach(a=>{const s=a;s.style.cursor="pointer",s.addEventListener("click",N)}),n.querySelectorAll(".financial-card__close").forEach(a=>{a.addEventListener("click",k)})}function ne(t){const e=document.createElement("div"),n=t.sum>=0?"financial-card--incomes":"financial-card--expenses";return e.className=`financial-card ${n}`,e.dataset.budget=t.label,e.dataset.sum=String(t.sum),e.dataset.incomes=String(t.incomes),e.dataset.expenses=String(t.expenses),e.dataset.orcamento=String(t.orcamento),e.innerHTML=`
  ...
  `)}function $(t){return t.toLocaleString("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:2,maximumFractionDigits:2})}class oe{constructor(e){w(this,"container");w(this,"entries",[]);w(this,"chartData",[]);const n=document.getElementById(e);if(!n)throw new Error(`Container ${e} não encontrado`);this.container=n}render(e){if(this.entries=e,this.chartData=this.calculateExpenseData(),this.chartData.length===0){this.renderEmptyState();return}this.renderChart()}calculateExpenseData(){const e=["TRANSFERÊNCIA","TRANSFERENCIA","SALDO","RENDA","RECEITA"],n=this.entries.filter(a=>{const s=a.tipo.toUpperCase().trim();return!e.includes(s)&&a.valor<0});if(n.length===0)return[];const i=new Map;for(const a of n){const s=a.tipo||"Sem Tipo",h=Math.abs(a.valor);i.set(s,(i.get(s)||0)+h)}const r=Array.from(i.values()).reduce((a,s)=>a+s,0);return Array.from(i.entries()).map(([a,s])=>({categoria:a,valor:s,percentual:s/r*100})).sort((a,s)=>s.valor-a.valor)}renderEmptyState(){this.container.innerHTML=`
    ...
    `}renderDonutChart(e){const s=e.reduce((d,l)=>d+l.valor,0);if(s===0)return"<p>Sem dados de despesas</p>";let h=-90;return`
      <svg 
        viewBox="0 0 200 200" 
        class="budget-chart__svg"
        style="max-width: 200px; max-height: 200px;"
      >
        ${e.map((d,l)=>{const y=d.valor/s*100/100*360,E=h,c=h+y;h=c;const p=E*Math.PI/180,g=c*Math.PI/180,u=100+85*Math.cos(p),f=100+85*Math.sin(p),_=100+85*Math.cos(g),L=100+85*Math.sin(g),q=y>180?1:0,z=["M 100 100",`L ${u} ${f}`,`A 85 85 0 ${q} 1 ${_} ${L}`,"Z"].join(" "),H=this.getColor(l);return`
        <path 
          d="${z}" 
          fill="${H}" 
          stroke="white" 
          ...
          data-value="${d.valor}"
        >
          <title>${d.categoria}: ${$(d.valor)} (${d.percentual.toFixed(1)}%)</title>
        </path>
      `}).join("")}
...
`;async function le(t,e){const n=document.querySelector(".details");if(!n)return;let i=e.map(m=>m.orcamento),r=t||[];const o=async(m,d=0)=>{try{if(console.log("[Details] Preparando gráfico de despesas por tipo..."),!document.getElementById("categoryBudgetChart"))if(d<3){console.warn(`[Details] Container categoryBudgetChart não encontrado (tentativa ${d+1}/3), aguardando...`),setTimeout(()=>o(m,d+1),200);return}else{console.warn("[Details] Container categoryBudgetChart não encontrado após múltiplas tentativas. Gráfico não será renderizado.");return}console.log("[Details] Container encontrado, carregando categorias completas...");const y=await j.getSheetCategoriesComplete();if(!y||y.length===0){console.log("[Details] Nenhuma categoria completa encontrada");return}console.log("[Details] Fazendo JOIN entre entries e categoriesComplete...");const E=new Map;for(const f of y)E.set(f.categoria.toLowerCase(),f.tipo);console.log("[Details] Categorias mapeadas:",Array.from(E.entries()));const c=r.filter(f=>m.includes(f.orcamento));console.log(`[Details] Filtrando ${r.length} entries por ${m.length} orçamentos -> ${c.length} entries`);const p=[...new Set(c.map(f=>f.categoria).filter(f=>f))];console.log("[Details] 📋 Categorias presentes nos entries filtrados:",p);const g=c.map(f=>{const _=(f.categoria||"").toLowerCase(),L=E.get(_)||"Sem Tipo";return{categoria:f.categoria||"",valor:f.valor||0,tipo:L}}),u=g.filter(f=>f.tipo==="Sem Tipo");if(u.length>0){console.warn(`[Details] ⚠️ Encontrados ${u.length} entries SEM TIPO:`),console.table(u.map(_=>({categoria:_.categoria,valor:_.valor,tipo:_.tipo})));const f=[...new Set(u.map(_=>_.categoria))];console.warn("[Details] 📋 Categorias SEM TIPO encontradas:",f),f.forEach(_=>{const L=E.has(_);console.warn(`[Details] Categoria "${_}" existe no mapa? ${L}`)})}console.log("[Details] Renderizando gráfico de despesas por tipo..."),ce("categoryBudgetChart",g),console.log("[Details] ✅ Gráfico de despesas por tipo renderizado com sucesso")}catch(v){console.error("[Details] Erro ao renderizar gráfico de despesas:",v)}},a=m=>{const d={};return m.forEach(l=>{!l.conta||l.conta.trim()===""||(d[l.conta]=(d[l.conta]||0)+(l.valor||0))}),Object.entries(d).map(([l,v])=>({conta:l,total:v}))},s=m=>{const d={};return m.forEach(l=>{const v=l.categoria||"Sem categoria";d[v]=(d[v]||0)+(l.valor||0)}),Object.entries(d).map(([l,v])=>({categoria:l,total:v}))},h=(m,d)=>{const l=n.querySelector("#detail-entries"),v=n.querySelector("#detail-entries-title"),y=n.querySelector("#entries-list");if(!l||!y||!v)return;v.innerHTML=`<span id="lancamentos">Lançamentos da Categoria: ${m}</span>`;const E=r.filter(c=>d.includes(c.orcamento)&&(c.categoria||"Sem categoria")===m&&c.valor<0);if(E.sort((c,p)=>{if(!c.data&&!p.data)return 0;if(!c.data)return 1;if(!p.data)return-1;const g=new Date(c.data).getTime(),u=new Date(p.data).getTime();return isNaN(g)&&isNaN(u)?0:isNaN(g)?1:isNaN(u)?-1:u-g}),l.classList.remove("details__category-entries--hidden"),y.innerHTML="",E.length===0){y.innerHTML='<p class="category-entries-empty">Nenhum lançamento encontrado nesta categoria.</p>';return}E.forEach(c=>{const p=document.createElement("div");p.className="category-entry-card";let g="--";if(c.data&&typeof c.data=="number"&&c.data>0){const u=M(c.data,!0);u&&(g=u.toLocaleDateString("pt-BR")+" "+u.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}))}p.innerHTML=`
      ...
      `,y.appendChild(p)}),setTimeout(()=>{const c=document.getElementById("lancamentos");c&&c.scrollIntoView({behavior:"smooth",block:"start"})},100)},C=(m,d)=>{const l=n.querySelector("#detail-entries"),v=n.querySelector("#detail-entries-title"),y=n.querySelector("#entries-list");if(!l||!y||!v)return;v.innerHTML=`<span id="lancamentos">Lançamentos da Conta: ${m}</span>`;const E=r.filter(c=>d.includes(c.orcamento)&&c.conta===m);if(E.sort((c,p)=>{if(!c.data&&!p.data)return 0;if(!c.data)return 1;if(!p.data)return-1;const g=new Date(c.data).getTime(),u=new Date(p.data).getTime();return isNaN(g)&&isNaN(u)?0:isNaN(g)?1:isNaN(u)?-1:u-g}),l.classList.remove("details__category-entries--hidden"),y.innerHTML="",E.length===0){y.innerHTML='<p class="category-entries-empty">Nenhum lançamento encontrado nesta conta.</p>';return}E.forEach(c=>{const p=document.createElement("div");p.className="category-entry-card";let g="--";if(c.data&&typeof c.data=="number"&&c.data>0){const u=M(c.data,!0);u&&(g=u.toLocaleDateString("pt-BR")+" "+u.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}))}p.innerHTML=`
        <div class="category-entry-card__date">${g}</div>
        <div class="category-entry-card__description">${c.descricao||"Sem descrição"}</div>
        <div class="category-entry-card__value">${S(c.valor||0)}</div>
      `,y.appendChild(p)}),setTimeout(()=>{const c=document.getElementById("lancamentos");c&&c.scrollIntoView({behavior:"smooth",block:"start"})},100)},b=async m=>{const d=Array.isArray(m)?m:[m];n.innerHTML=de,n.style.display="";const l=n.querySelector("#detail-saldo"),v=n.querySelector("#detail-accounts-cards"),y=n.querySelector("#detail-categories-cards"),E=r.filter(p=>d.includes(p.orcamento));if(!E.length){l&&(l.textContent=S(0)),await o(d);return}const c=E.reduce((p,g)=>p+(g.valor||0),0);l&&(l.textContent=S(c)),v&&(v.innerHTML="",a(E).forEach(({conta:p,total:g})=>{const u=document.createElement("div");u.className="details__card details__card--clickable",u.dataset.conta=p,u.innerHTML=`
          <div class="details__card-title">${p}</div>
          <div class="details__card-value">${S(g)}</div>
        `,u.addEventListener("click",()=>{v.querySelectorAll(".details__card").forEach(_=>{_.classList.remove("details__card--selected")});const f=n.querySelector("#detail-categories-cards");f&&f.querySelectorAll(".category-card").forEach(_=>{_.classList.remove("category-card--selected")}),u.classList.add("details__card--selected"),C(p,d)}),v.appendChild(u)})),y&&(y.innerHTML="",s(E).filter(g=>g.total<0).sort((g,u)=>g.total-u.total).slice(0,10).forEach((g,u)=>{const f=document.createElement("div");f.className="category-card",f.dataset.categoria=g.categoria,f.innerHTML=`
        ...
        `,f.addEventListener("click",()=>{y.querySelectorAll(".category-card").forEach(_=>{_.classList.remove("category-card--selected")}),v&&v.querySelectorAll(".details__card").forEach(_=>{_.classList.remove("details__card--selected")}),f.classList.add("category-card--selected"),h(g.categoria,d)}),y.appendChild(f)})),await o(d)};i.length>0?await b(i):await o([]),document.addEventListener("detail:show",async m=>{const l=m.detail.orcamento;i.includes(l)?i=i.filter(v=>v!==l):i.push(l),await b(i)}),document.addEventListener("cards:updated",async m=>{const d=m,{allEntries:l}=d.detail||{};l&&(r=l,await b(i))})}function ue(t=[],e="orcamento",n=-5,i=35){const{startSerial:r,endSerial:o}=G(n,i);return t.filter(a=>{const s=a&&a[e];return typeof s=="number"&&!Number.isNaN(s)&&s>=r&&s<=o})}function D(t=[],e="orcamento"){const n=new Map;return t.forEach(r=>{let o=r&&r[e],a=Number(o);if(!Number.isFinite(a)&&typeof o=="string"&&o.includes("/")){const C=o.split("/");if(C.length===3){const[b,m,d]=C.map(l=>Number(l));[b,m,d].every(Number.isFinite)&&(a=T(new Date(d,m-1,b)))}else if(C.length===2){const[b,m]=C.map(d=>Number(d));[b,m].every(Number.isFinite)&&(a=T(new Date(m,b-1,1)))}}if(!Number.isFinite(a))return;const s=Number(r.valor)||0,h=n.get(a)||{orcamento:a,sum:0,count:0,incomes:0,expenses:0};h.sum+=s,h.count+=1,s>=0?h.incomes+=s:h.expenses+=s,n.set(a,h)}),Array.from(n.values()).map(r=>({orcamento:r.orcamento,label:R(r.orcamento),count:r.count,sum:Number(r.sum.toFixed(2)),incomes:Number(r.incomes.toFixed(2)),expenses:Number(r.expenses.toFixed(2))})).sort((r,o)=>o.orcamento-r.orcamento)}function I(t=[],e="orcamento"){const n=r=>{let o=Number(r);if(Number.isFinite(o))return o;if(typeof r=="string"&&r.includes("/")){const a=r.split("/");if(a.length===3){const[s,h,C]=a.map(b=>Number(b));if([s,h,C].every(Number.isFinite))return T(new Date(C,h-1,s))}else if(a.length===2){const[s,h]=a.map(C=>Number(C));if([s,h].every(Number.isFinite))return T(new Date(h,s-1,1))}}return null},i=new Map;return t.forEach(r=>{const o=r&&r[e],a=n(o);a===null||!Number.isFinite(a)||i.has(a)||i.set(a,{orcamento:a,label:R(a)})}),Array.from(i.values()).sort((r,o)=>r.orcamento-o.orcamento)}async function F(){if(!await O()){console.warn("⚠️ Token inválido ou usuário não autenticado");return}if(te(),se(),await Y(()=>{console.log("✅ Lançamento adicionado! Recarregue a página para ver as mudanças."),window.location.reload()}),await U(()=>{console.log("✅ Lançamento futuro adicionado! Recarregue a página para ver as mudanças."),window.location.reload()}),await W(()=>{console.log("✅ Transferência realizada! Recarregue a página para ver as mudanças."),window.location.reload()}),J(()=>ee(),()=>Q(),()=>K()),!A.authStore.isValid){console.log("⚠️ Usuário não autenticado");return}if(!(await ge()).isValid){me();return}await pe(),console.log("✅ Dashboard inicializado")}async function ge(){try{const e=await(await fetch(`${X.configStatus}`,{method:"GET",headers:{Authorization:`Bearer ${A.authStore.token}`}})).json();return{isValid:e.hasRefreshToken&&e.hasSheetId}}catch(t){return console.error("Erro ao verificar configuração:",t),{isValid:!1}}}function me(){const t=document.getElementById("configBtn");t&&(t.style.display="");const e=document.getElementById("summaryCards");e&&(e.style.display="none");const n=document.querySelector(".dashboard__col--right.details");n&&(n.style.display="none");const i=document.getElementById("openEntryModal");i&&(i.style.display="none");const r=document.querySelector(".dashboard__header");if(r&&!document.getElementById("configMessage")){const o=document.createElement("p");o.id="configMessage",o.style.marginTop="1rem",o.textContent='Integração com Google não configurada. Clique em "Configurar Integração" para continuar.',r.appendChild(o);const a=document.createElement("a");a.href="/dashboard/configuracao.html",a.className="button primary",a.style.marginTop="1rem",a.style.display="inline-block",a.textContent="⚙️ Configurar Integração",r.appendChild(a)}}async function pe(){try{const t=await Z.fetchEntries(0,!1),e=(t==null?void 0:t.entries)??[];if(!e||e.length===0){fe();return}window.allEntries=e;const n=D(e);window.allBudgets=I(e);const i=ue(e),r=D(i),o=I(i);window.filteredEntries=i,window.summaryByBudget=r,window.budgetsInInterval=o;const a={};r.forEach(s=>{a[s.label]=!0}),ae(n,a),await le(e,o)}catch(t){console.error("Erro ao carregar dados:",t),ie("Não foi possível carregar os dados. Verifique sua conexão e tente novamente.")}}function fe(){const t=document.getElementById("summaryCards");t&&(t.style.display="none");const e=document.querySelector(".dashboard__col--right.details");e&&(e.style.display="none");const n=document.getElementById("openEntryModal");n&&(n.style.display="");const i=document.querySelector(".dashboard__header");if(i&&!document.getElementById("firstEntryMessage")){const r=document.createElement("div");r.id="firstEntryMessage",r.style.marginTop="1rem",r.className="notice",r.textContent='Você ainda não tem lançamentos. Insira o primeiro lançamento — ex. "Saldo inicial Banco Laranjinha" ou "Fatura cartão roxinho atual". Após inserir recarregue a página.',i.appendChild(r)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",F):F();
