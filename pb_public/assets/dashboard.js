var H=Object.defineProperty;var P=(a,e,n)=>e in a?H(a,e,{enumerable:!0,configurable:!0,writable:!0,value:n}):a[e]=n;var w=(a,e,n)=>P(a,typeof e!="symbol"?e+"":e,n);import{v as O,p as B,A as V}from"./auth.js";import{SheetsService as X}from"./sheets.js";import{t as S,e as L,g as j,i as G,a as U,b as Y,c as W,l as J,o as Z,d as K,f as Q}from"./fab-menu.js";import{r as ee}from"./user-menu.js";function b(a){return Number(a).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}let x=[],T={};function ae(a,e){const n=document.getElementById("summaryCards");if(!n)return;x=a||[],T=e||{};const o=x.slice().sort((t,r)=>r.orcamento-t.orcamento);n.className="financial-cards",n.innerHTML="",o.forEach(t=>{T[t.label]===!0?n.appendChild(te(t)):n.appendChild(ne(t))}),n.querySelectorAll(".financial-card--inactive").forEach(t=>{const r=t;r.style.cursor="pointer",r.addEventListener("click",M)}),n.querySelectorAll(".financial-card__close").forEach(t=>{t.addEventListener("click",F)})}function te(a){const e=document.createElement("div"),n=a.sum>=0?"financial-card--incomes":"financial-card--expenses";return e.className=`financial-card ${n}`,e.dataset.budget=a.label,e.dataset.sum=String(a.sum),e.dataset.incomes=String(a.incomes),e.dataset.expenses=String(a.expenses),e.dataset.orcamento=String(a.orcamento),e.innerHTML=`
    ...
    <div class="financial-card__value">${b(a.sum)}</div>
    ...
    <div class="financial-card__details financial-card__details--hidden">
      <div class="financial-card__detail">Receitas: ${b(a.incomes)}</div>
      <div class="financial-card__detail">Despesas: ${b(a.expenses)}</div>
    </div>
  ...
  `,e}function M(){const a=this.dataset.budget,e=parseFloat(this.dataset.sum),n=parseFloat(this.dataset.incomes),o=parseFloat(this.dataset.expenses),s=Number(this.dataset.orcamento);this.classList.remove("financial-card--inactive");const i=e>=0?"financial-card--incomes":"financial-card--expenses";this.classList.add(i),this.style.cursor="default",this.innerHTML=`
    ...
    <div class="financial-card__value">${b(e)}</div>
  ...
  `)}function $(a){return a.toLocaleString("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:2,maximumFractionDigits:2})}class oe{constructor(e){w(this,"container");w(this,"entries",[]);w(this,"chartData",[]);const n=document.getElementById(e);if(!n)throw new Error(`Container ${e} não encontrado`);this.container=n}render(e){if(this.entries=e,this.chartData=this.calculateExpenseData(),this.chartData.length===0){this.renderEmptyState();return}this.renderChart()}calculateExpenseData(){const e=["TRANSFERÊNCIA","TRANSFERENCIA","SALDO","RENDA","RECEITA"],n=this.entries.filter(t=>{const r=t.tipo.toUpperCase().trim();return!e.includes(r)&&t.valor<0});if(n.length===0)return[];const o=new Map;for(const t of n){const r=t.tipo||"Sem Tipo",g=Math.abs(t.valor);o.set(r,(o.get(r)||0)+g)}const s=Array.from(o.values()).reduce((t,r)=>t+r,0);return Array.from(o.entries()).map(([t,r])=>({categoria:t,valor:r,percentual:r/s*100})).sort((t,r)=>r.valor-t.valor)}renderEmptyState(){this.container.innerHTML=`
    ...
    `}renderDonutChart(e){const r=e.reduce((u,h)=>u+h.valor,0);if(r===0)return"<p>Sem dados de despesas</p>";let g=-90;return`
      <svg 
        viewBox="0 0 200 200" 
        class="budget-chart__svg"
        style="max-width: 200px; max-height: 200px;"
      >
        ${e.map((u,h)=>{const E=u.valor/r*100/100*360,v=g,p=g+E;g=p;const f=v*Math.PI/180,m=p*Math.PI/180,y=100+85*Math.cos(f),C=100+85*Math.sin(f),A=100+85*Math.cos(m),R=100+85*Math.sin(m),k=E>180?1:0,z=["M 100 100",`L ${y} ${C}`,`A 85 85 0 ${k} 1 ${A} ${R}`,"Z"].join(" "),q=this.getColor(h);return`
        <path 
          d="${z}" 
          fill="${q}" 
          stroke="white" 
...
`;async function de(a,e){const n=document.querySelector(".details");if(!n)return;let o=e.map(d=>d.orcamento),s=a||[];const i=async(d,l=0)=>{try{if(console.log("[Details] Preparando gráfico de despesas por tipo..."),!document.getElementById("categoryBudgetChart"))if(l<3){console.warn(`[Details] Container categoryBudgetChart não encontrado (tentativa ${l+1}/3), aguardando...`),setTimeout(()=>i(d,l+1),200);return}else{console.warn("[Details] Container categoryBudgetChart não encontrado após múltiplas tentativas. Gráfico não será renderizado.");return}console.log("[Details] Container encontrado, carregando categorias completas...");const h=await X.getSheetCategoriesComplete();if(!h||h.length===0){console.log("[Details] Nenhuma categoria completa encontrada");return}console.log("[Details] Fazendo JOIN entre entries e categoriesComplete...");const _=new Map;for(const m of h)_.set(m.categoria.toLowerCase(),m.tipo);console.log("[Details] Categorias mapeadas:",Array.from(_.entries()));const E=s.filter(m=>d.includes(m.orcamento));console.log(`[Details] Filtrando ${s.length} entries por ${d.length} orçamentos -> ${E.length} entries`);const v=[...new Set(E.map(m=>m.categoria).filter(m=>m))];console.log("[Details] 📋 Categorias presentes nos entries filtrados:",v);const p=E.map(m=>{const y=(m.categoria||"").toLowerCase(),C=_.get(y)||"Sem Tipo";return{categoria:m.categoria||"",valor:m.valor||0,tipo:C}}),f=p.filter(m=>m.tipo==="Sem Tipo");if(f.length>0){console.warn(`[Details] ⚠️ Encontrados ${f.length} entries SEM TIPO:`),console.table(f.map(y=>({categoria:y.categoria,valor:y.valor,tipo:y.tipo})));const m=[...new Set(f.map(y=>y.categoria))];console.warn("[Details] 📋 Categorias SEM TIPO encontradas:",m),m.forEach(y=>{const C=_.has(y);console.warn(`[Details] Categoria "${y}" existe no mapa? ${C}`)})}console.log("[Details] Renderizando gráfico de despesas por tipo..."),ie("categoryBudgetChart",p),console.log("[Details] ✅ Gráfico de despesas por tipo renderizado com sucesso")}catch(u){console.error("[Details] Erro ao renderizar gráfico de despesas:",u)}},t=d=>{const l={};return d.forEach(c=>{!c.conta||c.conta.trim()===""||(l[c.conta]=(l[c.conta]||0)+(c.valor||0))}),Object.entries(l).map(([c,u])=>({conta:c,total:u}))},r=d=>{const l={};return d.forEach(c=>{const u=c.categoria||"Sem categoria";l[u]=(l[u]||0)+(c.valor||0)}),Object.entries(l).map(([c,u])=>({categoria:c,total:u}))},g=async d=>{const l=Array.isArray(d)?d:[d];n.innerHTML=ce,n.style.display="";const c=n.querySelector("#detail-saldo"),u=n.querySelector("#detail-accounts-cards"),h=n.querySelector("#detail-categories-cards"),_=s.filter(v=>l.includes(v.orcamento));if(!_.length){c&&(c.textContent=b(0)),await i(l);return}const E=_.reduce((v,p)=>v+(p.valor||0),0);c&&(c.textContent=b(E)),u&&(u.innerHTML="",t(_).forEach(({conta:v,total:p})=>{const f=document.createElement("div");f.className="details__card details__card--clickable",f.dataset.conta=v,f.innerHTML=`
          <div class="details__card-title">${v}</div>
          <div class="details__card-value">${b(p)}</div>
        `,f.addEventListener("click",()=>{window.location.href=`/dashboard/lancamentos.html?conta=${encodeURIComponent(v)}`}),u.appendChild(f)})),h&&(h.innerHTML="",r(_).filter(p=>p.total<0).sort((p,f)=>p.total-f.total).slice(0,10).forEach((p,f)=>{const m=document.createElement("div");m.className="category-card",m.dataset.categoria=p.categoria,m.innerHTML=`
          <div class="category-card__rank">#${f+1}</div>
          <div class="category-card__content">
            <div class="category-card__name">${p.categoria}</div>
            <div class="category-card__value">${b(p.total)}</div>
          </div>
        `,m.addEventListener("click",()=>{window.location.href=`/dashboard/lancamentos.html?categoria=${encodeURIComponent(p.categoria)}`}),h.appendChild(m)})),await i(l)};o.length>0?await g(o):await i([]),document.addEventListener("detail:show",async d=>{const c=d.detail.orcamento;o.includes(c)?o=o.filter(u=>u!==c):o.push(c),await g(o)}),document.addEventListener("cards:updated",async d=>{const l=d,{allEntries:c}=l.detail||{};c&&(s=c,await g(o))})}function le(a=[],e="orcamento",n=-5,o=35){const{startSerial:s,endSerial:i}=j(n,o);return a.filter(t=>{const r=t&&t[e];return typeof r=="number"&&!Number.isNaN(r)&&r>=s&&r<=i})}function I(a=[],e="orcamento"){const n=new Map;return a.forEach(s=>{let i=s&&s[e],t=Number(i);if(!Number.isFinite(t)&&typeof i=="string"&&i.includes("/")){const d=i.split("/");if(d.length===3){const[l,c,u]=d.map(h=>Number(h));[l,c,u].every(Number.isFinite)&&(t=S(new Date(u,c-1,l)))}else if(d.length===2){const[l,c]=d.map(u=>Number(u));[l,c].every(Number.isFinite)&&(t=S(new Date(c,l-1,1)))}}if(!Number.isFinite(t))return;const r=Number(s.valor)||0,g=n.get(t)||{orcamento:t,sum:0,count:0,incomes:0,expenses:0};g.sum+=r,g.count+=1,r>=0?g.incomes+=r:g.expenses+=r,n.set(t,g)}),Array.from(n.values()).map(s=>({orcamento:s.orcamento,label:L(s.orcamento),count:s.count,sum:Number(s.sum.toFixed(2)),incomes:Number(s.incomes.toFixed(2)),expenses:Number(s.expenses.toFixed(2))})).sort((s,i)=>i.orcamento-s.orcamento)}function D(a=[],e="orcamento"){const n=s=>{let i=Number(s);if(Number.isFinite(i))return i;if(typeof s=="string"&&s.includes("/")){const t=s.split("/");if(t.length===3){const[r,g,d]=t.map(l=>Number(l));if([r,g,d].every(Number.isFinite))return S(new Date(d,g-1,r))}else if(t.length===2){const[r,g]=t.map(d=>Number(d));if([r,g].every(Number.isFinite))return S(new Date(g,r-1,1))}}return null},o=new Map;return a.forEach(s=>{const i=s&&s[e],t=n(i);t===null||!Number.isFinite(t)||o.has(t)||o.set(t,{orcamento:t,label:L(t)})}),Array.from(o.values()).sort((s,i)=>s.orcamento-i.orcamento)}async function N(){if(!await O()){console.warn("⚠️ Token inválido ou usuário não autenticado");return}if(ee(),se(),await G(()=>{console.log("✅ Lançamento adicionado! Recarregue a página para ver as mudanças."),window.location.reload()}),await U(()=>{console.log("✅ Lançamento futuro adicionado! Recarregue a página para ver as mudanças."),window.location.reload()}),await Y(()=>{console.log("✅ Transferência realizada! Recarregue a página para ver as mudanças."),window.location.reload()}),W(()=>Q(),()=>K(),()=>Z()),!B.authStore.isValid){console.log("⚠️ Usuário não autenticado");return}if(!(await ue()).isValid){ge();return}await me(),console.log("✅ Dashboard inicializado")}async function ue(){try{const e=await(await fetch(`${V.configStatus}`,{method:"GET",headers:{Authorization:`Bearer ${B.authStore.token}`}})).json();return{isValid:e.hasRefreshToken&&e.hasSheetId}}catch(a){return console.error("Erro ao verificar configuração:",a),{isValid:!1}}}function ge(){const a=document.getElementById("configBtn");a&&(a.style.display="");const e=document.getElementById("summaryCards");e&&(e.style.display="none");const n=document.querySelector(".dashboard__col--right.details");n&&(n.style.display="none");const o=document.getElementById("openEntryModal");o&&(o.style.display="none");const s=document.querySelector(".dashboard__header");if(s&&!document.getElementById("configMessage")){const i=document.createElement("p");i.id="configMessage",i.style.marginTop="1rem",i.textContent='Integração com Google não configurada. Clique em "Configurar Integração" para continuar.',s.appendChild(i);const t=document.createElement("a");t.href="/dashboard/configuracao.html",t.className="button primary",t.style.marginTop="1rem",t.style.display="inline-block",t.textContent="⚙️ Configurar Integração",s.appendChild(t)}}async function me(){try{const a=await J.fetchEntries(0,!1),e=(a==null?void 0:a.entries)??[];if(!e||e.length===0){pe();return}window.allEntries=e;const n=I(e);window.allBudgets=D(e);const o=le(e),s=I(o),i=D(o);window.filteredEntries=o,window.summaryByBudget=s,window.budgetsInInterval=i;const t={};s.forEach(r=>{t[r.label]=!0}),ae(n,t),await de(e,i)}catch(a){console.error("Erro ao carregar dados:",a),re("Não foi possível carregar os dados. Verifique sua conexão e tente novamente.")}}function pe(){const a=document.getElementById("summaryCards");a&&(a.style.display="none");const e=document.querySelector(".dashboard__col--right.details");e&&(e.style.display="none");const n=document.getElementById("openEntryModal");n&&(n.style.display="");const o=document.querySelector(".dashboard__header");if(o&&!document.getElementById("firstEntryMessage")){const s=document.createElement("div");s.id="firstEntryMessage",s.style.marginTop="1rem",s.className="notice",s.textContent='Você ainda não tem lançamentos. Insira o primeiro lançamento — ex. "Saldo inicial Banco Laranjinha" ou "Fatura cartão roxinho atual". Após inserir recarregue a página.',o.appendChild(s)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",N):N();
