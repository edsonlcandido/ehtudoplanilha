...
/**
 * Notifica o app Android (via JS Interface) que o login completou,
 * passando o pbToken + dados do user. Se nao tiver app host
 * (ex: rodando no browser normal), nao faz nada.
 */
function notifyAppAuth() {
    // @ts-ignore - window.AppAuth eh injetado pelo app Android
    if (typeof window.AppAuth !== 'undefined' && window.AppAuth?.onLoginSuccess) {
        try {
            // @ts-ignore
            window.AppAuth.onLoginSuccess(JSON.stringify({
            ...
            // Silencioso - falha no notify nao impede o login web
            console.warn('[Login] Falha ao notificar app:', e)
        }
    }
}
